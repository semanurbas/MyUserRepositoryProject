...
import springBoot.myUserServiceProject.message.request.LoginForm;
import springBoot.myUserServiceProject.message.request.SignUpForm;
import springBoot.myUserServiceProject.message.response.JwtResponse;
import springBoot.myUserServiceProject.message.response.ResponseMessage;
...
public class AuthRestAPIs {
    ...
    public ResponseEntity<?> registerUser(@Valid @RequestBody SignUpForm signUpRequest) {
        if (userRepository.existsByUsername(signUpRequest.getUsername())) {
        	log.error("user failed to register. Username is already taken!");
            return new ResponseEntity<>(new ResponseMessage("Fail -> Username is already taken!"),
                    HttpStatus.BAD_REQUEST);
        }

        if (userRepository.existsByEmail(signUpRequest.getEmail())) {
        	log.error("user failed to register.Email is already in use!");
